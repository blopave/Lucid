...
res={}
for name in ['informe-tokenizacion-lucid','roadmap-tokenizacion-inmuebles','estrategia-venta-inmuebles-tokenizados']:
    if not os.path.exists(os.path.join(OUT,name+'.pdf')): continue
    r=pypdf.PdfReader(os.path.join(OUT,name+'.pdf')); pages={}
    for i,p in enumerate(r.pages):
        for sid in re.findall(r'@@([a-z0-9-]+)@@',unicodedata.normalize('NFKC',p.extract_text() or '')):
